...
list_census_regions('CA16')
regions <- list_census_regions("CA16")# Get all census regions
csd_regions <- regions[regions$level == "CSD", ]  # Filter for Census Subdivisions (CSDs)
print(csd_regions)
...
# create a folder
output_folder <- "census_data16"  # Change to your desired path
if (!dir.exists(output_folder)) {
  dir.create(output_folder, recursive = TRUE)
}


for (i in 1:nrow(csd_regions)) {
  csd_id <- csd_regions$region[i]  
  csd_name <- csd_regions$name[i]  
  print(csd_name)
  ...
  write.csv(data, file = file.path(output_folder, paste0(csd_name, "_16.csv")))
}
